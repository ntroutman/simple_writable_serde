...
public class SequenceFileAsBinaryInputFormat extends
	...
	public RecordReader<BytesWritable, BytesWritable> getRecordReader(InputSplit split,
		...
		debug("new reader: " + split);
		return new SequenceFileAsBinaryRecordReader(job, (FileSplit) split);
	}
	
	public static class SequenceFileReader extends SequenceFile.Reader {
		public SequenceFileReader(FileSystem fs, Path path, Configuration conf)
				throws IOException {
			super(fs, path, conf);
		}
	 ...
	 */
	public static class SequenceFileAsBinaryRecordReader implements
		...
		public SequenceFileAsBinaryRecordReader(Configuration conf, FileSplit split)
			...
			debug("Create reader: " + path);

			FileSystem fs = path.getFileSystem(conf);
			this.in = new SequenceFileReader(fs, path, conf);
		...
		public float getProgress() throws IOException {
			if (end == start) {
				...
				return Math
						.min(1.0f, (float) ((in.getPosition() - start) / (double) (end - start)));
			}
		}
	}
